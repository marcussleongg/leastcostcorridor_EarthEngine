...
var clippedCostAnaya = anayaCost.clip(roi);
var clippedCostTobler = tobCost.clip(roi);
...
var edges = clippedCostAnaya.convolve(laplacian);
...
function three_raster_lcc_anaya(startPoint, endPoint) {
  ...
  var cCostFromStart = clippedCostAnaya.cumulativeCost(startPointImage, 250000, false);
  var cCostFromEnd = clippedCostAnaya.cumulativeCost(endPointImage, 250000, false);
  ...
  geometry: clippedCostAnaya.geometry(),
  scale: 30 * scale_up1,
...
// Perform least cost corridor search with Toblers cost
function three_raster_lcc_tobler(startPoint, endPoint) {
  var startPointImage = startPoint
  .map(function(f) { return f.set('constant', 1); }) // assign constant value
  .reduceToImage({
    properties: ['constant'],
  ...
  // Run cumulative cost function
  var cCostFromStart = clippedCostTobler.cumulativeCost(startPointImage, 250000, false);
  var cCostFromEnd = clippedCostTobler.cumulativeCost(endPointImage, 250000, false);
  ...
  // Convert the raster path to a vector. This is more robust for visualization
  var startEndPathVector1 = startEndLeastCostCorridor1.selfMask().reduceToVectors({
  geometry: clippedCostTobler.geometry(),
  scale: 30 * scale_up1,
  geometryType: 'polygon',
  eightConnected: true,
  bestEffort: true
...
function add_path_layers(pathVector, label, myColor) {
  var startEndStyledPath = pathVector.style({
  color: myColor, // Bright green
...
add_path_layers(three_raster_lcc_anaya(startPoint, endPoint), 'Start-End Anaya', '#00C425');
add_path_layers(three_raster_lcc_tobler(startPoint, endPoint), 'Start-End Tobler', '#00C2C6');
var lcp_tobler_qgis = ee.FeatureCollection('projects/ee-poleinikov/assets/lcp_tob_qgis_first');
Map.addLayer(lcp_tobler_qgis, {color: '#FF66E5'}, 'lcp_tobler_qgis');
var lcp_anaya_qgis = ee.FeatureCollection('projects/ee-poleinikov/assets/lcp_anaya_qgis_first');
Map.addLayer(lcp_anaya_qgis, {color: '#66A3FF'}, 'lcp_anaya_qgis');
var lcp_tobler_arcgis = ee.FeatureCollection('projects/ee-poleinikov/assets/lcp_tobler_arcgis_first');
Map.addLayer(lcp_tobler_arcgis, {color: '#B09300'}, 'lcp_tobler_arcgis');
var lcp_anaya_arcgis = ee.FeatureCollection('projects/ee-poleinikov/assets/lcp_anaya_arcgis_first');
Map.addLayer(lcp_anaya_arcgis, {color: '#F26E6E'}, 'lcp_anaya_arcgis');
