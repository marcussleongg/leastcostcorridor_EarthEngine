...
function computeToblersCost (slope, waterMask) {
  ...
  function three_multiraster_lcc(startPoint, endPoint) {
  ...
  Map.addLayer(three_multiraster_lcc(startPoint, point1), {}, 'Least-Cost Path (Styled Vector) Start-1');
  ...
  Map.addLayer(three_multiraster_lcc(point1, point2), {}, 'Least-Cost Path (Styled Vector) 1-2');
  var point3Geom = ee.Geometry.Point(10.7790069173826, 44.6540878517765);
  var point3 = ee.FeatureCollection(point3Geom);
  Map.addLayer(three_multiraster_lcc(point2, point3), {}, 'Least-Cost Path (Styled Vector) 2-3');
  var point4Geom = ee.Geometry.Point(10.6298059531519, 44.6989918768963);
  var point4 = ee.FeatureCollection(point4Geom);
  Map.addLayer(three_multiraster_lcc(point3, point4), {}, 'Least-Cost Path (Styled Vector) 3-4');
  var point5Geom = ee.Geometry.Point(10.3254918017869, 44.8017940139929);
  var point5 = ee.FeatureCollection(point5Geom);
  Map.addLayer(three_multiraster_lcc(point4, point5), {}, 'Least-Cost Path (Styled Vector) 4-5');
  var point6Geom = ee.Geometry.Point(9.69696430808572, 45.0513627103219);
  var point6 = ee.FeatureCollection(point6Geom);
  Map.addLayer(three_multiraster_lcc(point5, point6), {}, 'Least-Cost Path (Styled Vector) 5-6');
  var point7Geom = ee.Geometry.Point(9.5029900028069, 45.311166366664);
  var point7 = ee.FeatureCollection(point7Geom);
  Map.addLayer(three_multiraster_lcc(point6, point7), {}, 'Least-Cost Path (Styled Vector) 6-7');
  var point8Geom = ee.Geometry.Point(9.32018746590149, 45.3553880723531);
  var point8 = ee.FeatureCollection(point8Geom);
  Map.addLayer(three_multiraster_lcc(point7, point8), {}, 'Least-Cost Path (Styled Vector) 7-8');
  Map.addLayer(three_multiraster_lcc(point8, endPoint), {}, 'Least-Cost Path (Styled Vector) 8-End');
