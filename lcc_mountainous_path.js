...
var initial_tolerance_percentage = 0.15;
// Controls how "flat" an area must be to be included
var lowGradientThreshold = 0.02;

var white = ee.Image.constant(1);
Map.addLayer(white, {min: 0, max: 1, palette: ['ffffff']}, 'White Layer');
var countries = ee.FeatureCollection("projects/ee-poleinikov/assets/countries_outline");
...
var startPointGeom = ee.Geometry.Point(7.51406905114773, 45.0721279374497);
var startPoint= ee.FeatureCollection(startPointGeom)
var endPointGeom = ee.Geometry.Point(6.58398976068974, 44.8412823899257);
...
function three_raster_lcc(startPoint, endPoint) {
  ...
  Map.addLayer(startEndPathVector1, {}, 'region of lcc 1');
  ...
  Map.addLayer(startEndPathVector2, {}, 'region of lcc 2');
...
var point1Geom = ee.Geometry.Point(7.36757694255264, 45.0959607751417);
var point1 = ee.FeatureCollection(point1Geom);
//add_path_layers(three_raster_lcc(startPoint, point1), 'Start-1');
var point2Geom = ee.Geometry.Point(7.1789646203616, 45.1270920065341);
var point2 = ee.FeatureCollection(point2Geom);
//add_path_layers(three_raster_lcc(point1, point2), '1-2');
var point3Geom = ee.Geometry.Point(7.15075101213495, 45.1391910655409);
var point3 = ee.FeatureCollection(point3Geom);
//add_path_layers(three_raster_lcc(point2, point3), '2-3');
var point4Geom = ee.Geometry.Point(7.04802589103844, 45.1386364554695);
var point4 = ee.FeatureCollection(point4Geom);
//add_path_layers(three_raster_lcc(point3, point4), '3-4');
var point5Geom = ee.Geometry.Point(6.9833093151297, 45.1179700828626);
var point5 = ee.FeatureCollection(point5Geom);
//add_path_layers(three_raster_lcc(point4, point5), '4-5');
var point6Geom = ee.Geometry.Point(6.92971034769875, 45.0980219034227);
var point6 = ee.FeatureCollection(point6Geom);
//add_path_layers(three_raster_lcc(point5, point6), '5-6');
var point7Geom = ee.Geometry.Point(6.833483926485, 45.0335382534053);
var point7 = ee.FeatureCollection(point7Geom);
//add_path_layers(three_raster_lcc(point6, point7), '6-7');
var point8Geom = ee.Geometry.Point(6.79316951811283, 44.9541104879673);
var point8 = ee.FeatureCollection(point8Geom);
//add_path_layers(three_raster_lcc(point7, point8), '7-8');
var point9Geom = ee.Geometry.Point(6.72171521067472, 44.930412882357);
var point9 = ee.FeatureCollection(point8Geom);
//add_path_layers(three_raster_lcc(point7, point8), '7-8');
var point10Geom = ee.Geometry.Point(6.71896125917504, 44.928504575483);
var point10 = ee.FeatureCollection(point8Geom);
//add_path_layers(three_raster_lcc(point7, point8), '7-8');
var point11Geom = ee.Geometry.Point(6.64403765689139, 44.8998315095777);
var point11 = ee.FeatureCollection(point8Geom);
//add_path_layers(three_raster_lcc(point7, point8), '7-8');
var point12Geom = ee.Geometry.Point(6.58398976068974, 44.8412823899257);
var point12 = ee.FeatureCollection(point8Geom);
//add_path_layers(three_raster_lcc(point7, point8), '7-8');
//add_path_layers(three_raster_lcc(point8, endPoint), '8-End');
add_path_layers(three_raster_lcc(startPoint, endPoint), 'Start-End');

var lcp_anaya_arcgis = ee.FeatureCollection("projects/ee-poleinikov/assets/finalized_lcp_anaya_arcgis");
Map.addLayer(lcp_anaya_arcgis, {color: 'red'}, "lcp_anaya_arcgis");
var lcp_anaya_qgis = ee.FeatureCollection("projects/ee-poleinikov/assets/finalized_lcp_anaya_qgis");
Map.addLayer(lcp_anaya_qgis, {color: 'red'}, "lcp_anaya_qgis");
var lcp_tobler_qgis = ee.FeatureCollection("projects/ee-poleinikov/assets/finalized_lcp_tobler_qgis");
Map.addLayer(lcp_tobler_qgis, {color: 'orange'}, "lcp_tobler_qgis");
var lcp_tobler_arcgis = ee.FeatureCollection("projects/ee-poleinikov/assets/finalized_lcp_tobler_arcgis");
Map.addLayer(lcp_tobler_arcgis, {color: 'orange'}, "lcp_tobler_arcgis");
...
Map.addLayer(point9, {color: 'blue'}, 'Point 9');
Map.addLayer(point10, {color: 'blue'}, 'Point 10');
Map.addLayer(point11, {color: 'blue'}, 'Point 11');
Map.addLayer(point12, {color: 'blue'}, 'Point 12');
